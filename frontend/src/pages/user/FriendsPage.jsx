...
import { 
  Search, UserCheck, MessageCircle, Crown, Sparkles, Award, 
  Briefcase, Filter, X, MapPin, Users, Star, ChevronDown, Plus 
} from "lucide-react";
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12">
          <div className="mb-6 md:mb-0">
            <div className="flex items-center mb-2">
              ...
              <h1 className="text-4xl font-light text-[#02182E] tracking-wide">Elite Network</h1>
            </div>
            <p className="text-[#022F56] font-light mt-3 ml-16">Connect with distinguished professionals</p>
          </div>
          <div className="flex items-center">
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-3 flex items-center shadow-lg border border-white/30">
              ...
              <span className="text-sm font-medium text-[#022F56] tracking-wide">PREMIUM NETWORK</span>
            </div>
          </div>
        </div>
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
                ...
                <div className="absolute z-20 w-full bg-white/95 backdrop-blur-md border border-[#85C4E4]/30 rounded-xl mt-2 max-h-60 overflow-y-auto shadow-xl">
                  {filteredUsers.slice(0, 5).map((user) => (
                    ...
                    >
                      ...
                      <div>
                        <p className="text-[#02182E] font-medium text-sm">{user.name}</p>
            ...
            <div className="w-full lg:w-2/5 relative">
              ...
              >
                ...
                <ChevronDown size={18} className={`text-[#488DB4] transition-transform duration-300 ${isFilterOpen ? 'rotate-180' : ''}`} />
              </button>
              ...
              {isFilterOpen && (
                <div className="absolute mt-2 w-full bg-white/95 backdrop-blur-md rounded-xl shadow-xl border border-[#85C4E4]/30 z-10 p-4">
                  <div className="flex justify-between items-center mb-3">
                    <h3 className="font-medium text-[#022F56] text-sm tracking-wide">FILTER BY PROFESSION</h3>
                    <button onClick={() => setIsFilterOpen(false)} className="text-[#488DB4] hover:text-[#022F56] transition-colors">
                      <X size={16} />
                    </button>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      className={`py-2.5 px-3 rounded-lg text-xs font-medium tracking-wide transition-all duration-200 ${selectedProfession === 'all' ? 'bg-gradient-to-r from-[#022F56] to-[#488DB4] text-white shadow-md' : 'bg-[#CCDEE4]/30 text-[#022F56] hover:bg-[#85C4E4]'}`}
                      onClick={() => {
                    ...
                    {professions.map(profession => (
                      <button
                        key={profession}
                        className={`py-2.5 px-3 rounded-lg text-xs font-medium tracking-wide transition-all duration-200 ${selectedProfession === profession ? 'bg-gradient-to-r from-[#022F56] to-[#488DB4] text-white shadow-md' : 'bg-[#CCDEE4]/30 text-[#022F56] hover:bg-[#85C4E4]'}`}
                        onClick={() => {
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 animate-pulse">
            {[...Array(8)].map((_, i) => (
              <div key={i} className="bg-white/60 backdrop-blur-sm rounded-2xl shadow-lg overflow-hidden h-96 border border-white/30">
          ...
          filteredUsers?.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {filteredUsers.map((user) => (
                <div key={user._id} className="group bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg overflow-hidden transition-all duration-500 hover:shadow-2xl hover:-translate-y-1.5 border border-white/30">
                  {/* Profile Header */}
                  <div className="relative">
                  ...
                  <div className="pt-14 pb-6 px-5 text-center">
                    <h3 className="font-semibold text-lg text-[#02182E] tracking-wide mb-1">{user.name}</h3>
                    ...
                    <button className="mt-3 w-full flex items-center justify-center text-[#488DB4] text-xs font-light hover:text-[#022F56] transition-colors duration-200">
            ...
            <div className="text-center py-20 bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl mt-6 border border-white/30">
              ...
              <h3 className="text-2xl font-light text-[#022F56] tracking-wide mb-2">No professionals found</h3>
              <p className="text-[#488DB4] font-light max-w-md mx-auto">
                Try adjusting your search criteria or explore all professionals in our network
              </p>
            </div>
          )
        )}
